...
class ExtendSidebar
{
    ...
    public function getTemplate($route): Response
    {
        ...
        if ($this->security->isGranted('liste-des-articles') || $this->security->isGranted('etiquette-darticles') || $this->security->isGranted('categories-darticles')) {
            $template .= '<li class="' . (str_contains((string) $route, "blog_post_") ? "active" : "") . '">';
            ...
            if ($this->security->isGranted('liste-des-articles')) {
                $template .= '<li class="' . (str_contains((string) $route, "blog_post_index") ? "active" : "") . '"><a href="' . $this->router->generate('blog_post_index') . '">Liste des articles</a></li>';
            }
            if ($this->security->isGranted('categories-darticles')) {
                $template .= '<li class="' . (str_contains((string) $route, "blog_post_category") ? "active" : "") . '"><a href="' . $this->router->generate('blog_post_category_index') . '">Catégories d\'articles</a></li>';
            }
            if ($this->security->isGranted('etiquette-darticles')) {
                $template .= '<li class="' . (str_contains((string) $route, "blog_post_tag") ? "active" : "") . '"><a href="' . $this->router->generate('blog_post_tag_index') . '">Étiquettes d\'articles</a></li>';
            }
            $template .= '</ul>';
            $template .= '</li>';
        }
    ...
    {
        $template = '<li class="' . (str_contains((string) $route, "blog_options") ? "active" : "") . '"><a href="' . $this->router->generate('blog_options') . '">Blog</a></li>';
        return new Response($template);
    }
}